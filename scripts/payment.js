...
function  upi_show(){
    ...
    <button onclick="orderplace()" id="upi-order"> PLACE ORDER</button>
    <div id="check">
 ...
 function credit_show(){
  ...
  <select name="" id="banks">
   ...
   function wallets_show(){
    ...
    //  console.log(cart_arr);
     ...
     function  appendcartItems(data){
       ...
       data.map(function (element,i){
        ...
        pricetag.innerText=`₹ ${element.discountPrice}`;
    ...
    var tp;
    var wd;
    tp=cartdata.reduce(function(ac,cv){
        return ac + Number(cv.discountPrice);
    },0)
    wd=cartdata.reduce(function(ac,cv){
        return ac + Number(cv.discountPrice);
    },0)
    ...
    document.querySelector(".tm_vam_3").textContent="₹"+(tp+disc);
     ...
     function orderplace(){
        ...
        var num =document.getElementById("upi-id").value;
